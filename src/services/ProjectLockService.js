...
export const ProjectLockService = {
  ...
  async acquireLock(projectId, userId) {
    ...
    // Se retornou dados, conseguimos o lock. Se array vazio, alguém já travou.
    return data && data.length > 0;
  },
  
  // Mantém o bloqueio ativo (chamar a cada 1 ou 2 min)
  ...
  async checkLockStatus(projectId, currentUserId) {
    ...
    if (data.locked_by && !isExpired && data.locked_by !== currentUserId) {
      return { isLocked: true, lockedBy: data.locked_by };
    }
