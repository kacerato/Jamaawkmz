...
const ProjectManager: React.FC<ProjectManagerProps> = ({
  ...
  const calculateProjectDistance = (project: Project): number => {
    ...
    for (let i = 1; i < project.points.length; i++) {
      ...
      if (current && previous && current.lat && current.lng && previous.lat && previous.lng) {
        ...
        const spans = (current.spans !== undefined && current.spans !== null && !isNaN(Number(current.spans))) ?
  ...
  const ProjectCard = ({ project }: { project: Project }) => {
    ...
    const totalSpans = project.points?.reduce((sum, point) => {
      const spans = (point.spans !== undefined && point.spans !== null && !isNaN(Number(point.spans))) ?
