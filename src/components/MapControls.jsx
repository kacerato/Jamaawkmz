...
import { LocateFixed, Layers, Moon, Sun, Globe, Map as MapIcon, Check } from 'lucide-react';
...
const MapControls = ({
  ...
  // Definição dos estilos disponíveis com ícones e cores
  const styles = [
  ...
  return (
    <>
      ...
      {/* A classe 'bottom-...' muda dinamicamente baseada no isTrackingActive */}
      <div 
      ...
      <div className="absolute top-24 right-4 z-20 flex flex-col items-end gap-2">
        ...
        {/* Menu Expandido (Aparece ao clicar) */}
        <div className={`
            ...
            <button
              ...
              className={`
                ...
                ${currentMapStyle === style.id 
                  ? 'bg-slate-800/95 border-cyan-500/50 translate-x-0 ring-1 ring-cyan-500/30' 
                  : 'bg-slate-900/80 border-white/10 hover:bg-slate-800 translate-x-2 hover:translate-x-0'}
              `}
            >
              ...
              <div className="flex-1">
                <span className={`block text-xs font-bold uppercase tracking-wider ${currentMapStyle === style.id ? 'text-white' : 'text-slate-300 group-hover:text-white'}`}>
                  {style.name}
                </span>
        ...
        <Button
          ...
          className={`
            ...
            ${showStyleMenu 
              ? 'bg-cyan-600 border-cyan-400 text-white rotate-90 shadow-cyan-500/30' 
              : 'bg-slate-900/90 backdrop-blur-xl border-white/20 text-cyan-400 hover:scale-105'}
          `}
          title="Alterar visual do mapa"
        >
