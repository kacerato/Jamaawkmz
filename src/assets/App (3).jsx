...
function App() {
  ...
  // Efeito para rastrear posição do usuário - SEMPRE ATIVO
  useEffect(() => {
    let watchId = null
    
    // Sempre ativar a localização, não apenas quando tracking estiver ativo
    if (navigator.geolocation) {
      watchId = navigator.geolocation.watchPosition(
        (position) => {
          ...
          // Se estiver no modo automático E tracking ativo, adiciona ponto automaticamente
        ...
        (error) => {
          ...
          // Tentar novamente com configuração menos restritiva
          if (navigator.geolocation) {
            navigator.geolocation.getCurrentPosition(
              (position) => {
                const { latitude, longitude } = position.coords
...
<kml xmlns="http://www.opengis.net/kml/2.2">
  ...
  return (
    <div className="relative h-screen w-screen overflow-hidden bg-slate-900">
      ...
      {/* Controles de Rastreamento - CORRIGIDOS */}
      {tracking && showTrackingControls && (
        <div className="fixed bottom-6 left-1/2 transform -translate-x-1/2 z-50 animate-slide-in-bottom">
          <div className="bg-gradient-to-r from-slate-800 to-slate-700 backdrop-blur-lg border border-slate-600/50 rounded-2xl shadow-2xl p-4 min-w-[350px] max-w-[90vw]">
            ...
            {/* Botões de controle - CORRIGIDO COM GRID */}
            <div className="tracking-controls-grid">
              {/* Botão Pausar/Retomar */}
              <Button
                onClick={pauseTracking}
                className={`tracking-button font-medium ${
                  paused 
